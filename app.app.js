/* app.app.js - Versión corregida para copia intermes y recuperación robusta.
   Recomendación: Exporta primero tus datos con el botón Exportar o con el snippet indicado.
*/
...
class Finanzas {
  ...
  // Busca la última clave de mes existente anterior a `key` en el dataset,
  // ordenando las claves y eligiendo la máxima que sea < key.
  findLastExistingBefore(key){
    const meses = Object.keys(this.data).filter(k => /^\d{4}-\d{2}$/.test(k)).sort();
    // buscar el índice del key; si no existe key, encontrar el mayor < key
    for(let i = meses.length-1; i>=0; i--){
      if(meses[i] < key) return meses[i];
    }
    return null;
  }

  // Asegura el mes y copia desde la última fecha existente (no solo mes anterior numérico)
  ensureMonth(key){
    if(this.data[key]) return;
    const prev = this.findLastExistingBefore(key);
    if(prev){
      ...
      Object.keys(copy).forEach(k=>{
        if(Array.isArray(copy[k])){
          copy[k] = copy[k].map(it=>{
            ...
            n.id = this.uid();
            // fecha: primer día del nuevo mes si el campo fecha existe
            if(n.fecha) n.fecha = `${key}-01`;
            // reset paid flag (no queremos arrastrar pago marcado)
  ...
  renderAll(){
    ...
    this.renderList("listaIngresos",d.ingresos,i=>this.rowGeneric("💵",i,"ingresos",i.monto,i.paid));
    ...
    this.renderList("listaCompras",d.gastosCompras,i=>this.rowGeneric("🛒",i,"gastosCompras",i.monto,i.paid));
