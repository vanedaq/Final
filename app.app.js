/* app.app.js - Organizador Financiero (versión completa corregida)
   - modal robusto (no se queda "pegado")
   - parseo de tasas con coma (1,84 -> 0.0184)
   - cuota francés + aval + iva-aval
   - copia de fijos/tarjetas/creditos/ahorros al crear nuevo mes
   - persistencia mes seleccionado
   - botón "Marcar como pagado" (toggle)
*/

/* ============== Utilidades ============== */
const fmt = (v) => new Intl.NumberFormat("es-CO",{style:"currency",currency:"COP",minimumFractionDigits:0}).format(v||0);

/** "1,84" -> 0.0184 (fracción mensual). Solo coma admitida. */
function parsePctComma(str){
  const s = String(str||"").trim();
  if(s==="") return 0;
  // permitir "1,84" o "1" o "1,8"
...
class Finanzas {
  constructor(){
    this.key="organizadorFinanciero_v2";
    this.iniYM="2025-08";
    // carga mes seleccionado persistido o inicial
    const sel = localStorage.getItem(this.key + "_mes");
    this.mes = sel || this.iniYM;
    this.data=this.load();

    this.cacheEls();
    this.bindUI();
    this.buildMonths();
    // asegurar que panel visible = dashboard al inicio
  ...
  bindUI(){
    ...
    if(this.sel) this.sel.addEventListener("change",(e)=>{ this.mes=e.target.value; localStorage.setItem(this.key + "_mes", this.mes); this.ensureMonth(this.mes); this.renderAll(); this.toast("Mes cambiado"); });
    ...
    // Delegación para editar/eliminar/pagar
    document.body.addEventListener("click",(ev)=>{
      const a=ev.target.closest("[data-action]");
      if(!a) return;
      ev.preventDefault();
      const act=a.dataset.action, key=a.dataset.key, id=a.dataset.id;
      if(!id) return;
      const nid = isNaN(Number(id)) ? id : Number(id);
      if(act==="edit") this.edit(key,nid);
      if(act==="del") this.del(key,nid);
      if(act==="pay") this.togglePaid(key,nid);
      if(act==="addsave") this.addAhorroMonto(nid);
    });

    // cerrar modal al hacer click fuera (overlay) o con Escape
    this.btns.modal.addEventListener("click",(e)=>{ if(e.target.id==="modal") this.closeModal(); });
    document.addEventListener("keydown",(e)=>{ if(e.key==="Escape") this.closeModal(); });

    // normalize numeric inputs on change (reemplaza punto por coma en tasas en inputs text)
    document.body.addEventListener('input', (e)=>{
      const el = e.target;
      if(!el) return;
      // para campos con attribute inputmode decimal, reemplazar puntos por coma en pantalla (la parseamos luego)
      if(el.getAttribute && el.getAttribute('inputmode')==='decimal'){
        // allow both comma and dot while typing, but keep dot->comma replacement requested earlier
  ...
  load(){
    try{ const raw=localStorage.getItem(this.key); if(raw) return JSON.parse(raw); }catch(e){ console.error(e); }
    // seed inicial si no existe nada
  ...
  ensureMonth(key){
    ...
    if(this.data[prev]){
      ...
      // reasign ids and dates but keep payment status (for tarjetas/creditos we want to copy recurring)
      Object.keys(copy).forEach(k=>{
        if(Array.isArray(copy[k])){
          copy[k] = copy[k].map(it=>{
            const n = Object.assign({}, it);
            n.id = this.uid();
            // colocar fecha al primer día del nuevo mes si existe
            n.fecha = `${key}-01`;
            // si es gasto fijo o tarjeta o credito: mantener paid = false para el nuevo mes
            if(typeof n.paid !== 'undefined') n.paid = false;
            return n;
          });
        }
      });
  ...
  /** Cuota francés + aval + IVA-aval
   * M: capital, i: tasa mensual (fracción), n: cuotas
   */
  ...
  renderAll(){
    ...
    this.renderList("listaFijos",d.gastosFijos,i=>this.rowGeneric("🏠",i,"gastosFijos",i.monto,i.paid));
  ...
  renderList(id,arr,row){ const el=document.getElementById(id); if(!el) return; el.innerHTML=arr && arr.length?arr.map(row).join(""):'<p class="meta">Sin registros.</p>'; }

  rowGeneric(icon,i,key,monto,paid=false){
    return `<div class="item ${paid?'is-paid':''}">
      ...
      <div class="actions">
        ${key ? `<a data-action="edit" data-key="${key}" data-id="${i.id}" href="#">✏️ Editar</a>` : ''}
        ${key ? `<a data-action="del" data-key="${key}" data-id="${i.id}" href="#">🗑️ Eliminar</a>` : ''}
        ${key ? `<a data-action="pay" data-key="${key}" data-id="${i.id}" href="#">${paid? '✅ Pagado':'🔘 Marcar pagado'}</a>` : ''}
      </div>
    </div>`;
  }
  rowTarjeta(i,key){
    return `<div class="item ${i.paid?'is-paid':''}">
      ...
      <div class="actions">
        ...
        <a data-action="pay" data-key="${key}" data-id="${i.id}" href="#">${i.paid? '✅ Pagado' : '🔘 Marcar pagado'}</a>
      </div>
    </div>`;
  }
  rowCredito(i,key){
  ...
  edit(key,id){
    ...
    if(!isDeuda && key!=="ahorros"){
      ...
      fields= f("nombre","text","Nombre",it.nombre)
            ...
            + f("tasa","text","Tasa mensual % (coma)",formatPctComma(it.tasaMensual,2),"inputmode='decimal' pattern='^\\d+(,\\d{1,3})?$' oninput='this.value=this.value.replace(\".\",\",\");'");
  ...
  showModal(title, innerHtml, onSubmit){
    ...
    // asegurar que el botón X cierra
    const xbtn = this.btns.closeModal;
    if(xbtn) xbtn.onclick = cancel;

    // Form submit: cerrar modal antes de ejecutar la callback para evitar bloqueo si hay error
    form.onsubmit=(e)=>{
      e.preventDefault();
      const vals={};
      ...
      // invocar onSubmit con try/catch
      setTimeout(()=>{
        try{
          onSubmit(vals);
        }catch(err){
          console.error('onSubmit error:', err);
          if(typeof this.toast === 'function') this.toast('Error guardando (ver consola)');
        }
      },0);
    };
  }
...
/* Parche adicional por si queda modal colgado (seguro) */
(function(){
  // delegación para cierre por elementos con clase modal-close
