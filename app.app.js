...
/** "1,84" -> 0.0184 (fracción mensual). Acepta coma como separador. */
function parsePctComma(str){
  const s = String(str||"").trim();
  if(!s) return NaN;
  // allow "1", "1,8", "1,84", "1,845"
  const normalized = s.replace(",",".").replace(/[^\d.]/g,"");
  if(!/^\d+(\.\d{1,})?$/.test(normalized)) return NaN;
...
class Finanzas {
  constructor(){
    ...
    this.keyMes="organizadorFinanciero_mes";
    this.iniYM="2025-08";
    this.mes = localStorage.getItem(this.keyMes) || this.iniYM;
    this.data=this.load();
  ...
  bindUI(){
    ...
    if(this.sel) this.sel.addEventListener("change",(e)=>{ this.mes=e.target.value; localStorage.setItem(this.keyMes,this.mes); this.ensureMonth(this.mes); this.renderAll(); this.toast("Mes cambiado"); });
    ...
    // Delegación para editar/eliminar/ahorrar/pagar
    document.body.addEventListener("click",(ev)=>{
      const a=ev.target.closest("a[data-action]"); if(!a) return;
      ev.preventDefault();
      const act=a.dataset.action, key=a.dataset.key, id=a.dataset.id;
      if(act==="edit") this.edit(key,Number(id));
      if(act==="del") this.del(key,Number(id));
      if(act==="addsave") this.addAhorroMonto(Number(id));
      if(act==="paid") this.togglePaid(key,Number(id));
    });
  ...
  ensureMonth(key){
    ...
    if(this.data[prev]){
      ...
      // Ajuste: solo copiar tarjetas y creditos y fijos (para siguiente mes)
      const keep=(arrName)=>{
        const src=this.data[prev][arrName]||[];
        return src.map(it=>{
          const clone=JSON.parse(JSON.stringify(it));
          clone.id=this.uid(); clone.fecha=`${key}-01`;
          // reset month-specific fields
          if(arrName==="tarjetas"||arrName==="creditos"){
            clone.cuotasPagadas = Math.max(0, Number(clone.cuotasPagadas || 0));
            clone.paid = false;
          }
          return clone;
        });
      ...
      this.data[key]={
        ingresos: [], // default empty (user adds)
        gastosFijos: keep("gastosFijos"),
        tarjetas: keep("tarjetas"),
        creditos: keep("creditos"),
        gastosCompras: [], // purchases don't copy
        ahorros: keep("ahorros"),
      };
    }else{
   ...
   *  M: monto, i: tasa mensual en fracción (ej 0.0184), n: cuotas
   */
  ...
  rowGeneric(icon,i,key,monto){
    return `<div class="item ${i.paid?'is-paid':''}">
      ...
      <div class="actions">
        <a data-action="paid" data-key="${key}" data-id="${i.id}" href="#">${i.paid? '✅ Pagado':'☐ Marcar pago'}</a>
  ...
  renderHistorial(){
    ...
    const rows=meses.map(m=>{
      ...
      return `<tr><td>${m}</td><td>${fmt(ing)}</td><td>${fmt(gas)}</td>
        <td style="color:${bal>=0?"#00b894":"#ff6b6b"}">${fmt(bal)}</td><td>${p.toFixed(1)}%</td></tr>`;
    }).join("");
  ...
  openForm(tipo,item=null){
    ...
    this.showModal(title, fields, (vals)=>{
      ...
      if(tipo==="ingreso"){
        ...
        if(!(tasa>0 && tasa<=0.5)) { this.toast("Tasa inválida (usa coma)"); return; }
  ...
  edit(key,id){
    ...
    this.showModal(title, fields, (vals)=>{
      ...
      if(!isDeuda && key!=="ahorros"){
        ...
        const tasa=pct(vals.tasa); if(!(tasa>0 && tasa<=0.5)){ this.toast("Tasa inválida"); return; }
        const M=n(vals.montoTotal), cu=parseInt(vals.numeroCuotas||0), pag=parseInt(vals.cuotasPagadas||0);
        ...
        const tasa=pct(vals.tasa), aval=pct(vals.aval||"0"), iva=pct(vals.ivaAval||"0");
        if(!(tasa>0 && tasa<=0.5)){ this.toast("Tasa inválida"); return; }
        const M=n(vals.montoTotal), cu=parseInt(vals.numeroCuotas||0), pag=parseInt(vals.cuotasPagadas||0);
  ...
  togglePaid(key,id){
    ...
    it.paid = !it.paid;
    this.save(); this.renderAll(); this.toast(it.paid ? "Marcado como pagado" : "Marcado como no pagado");
  }
  ...
  showModal(title, innerHtml, onSubmit){
    ...
    setTimeout(()=>{ const first=form.querySelector("input"); if(first) first.focus(); },50);
    ...
    form.onsubmit=(e)=>{
      ...
      // Close modal BEFORE running submit logic to avoid "modal pegado"
      this.closeModal();
      try{ onSubmit(vals); }catch(err){ console.error(err); this.toast("Error al guardar"); }
    };
  }
  closeModal(){
  ...
  reset(){ if(confirm("¿Borrar datos locales?")){ localStorage.removeItem(this.key); localStorage.removeItem(this.keyMes); location.reload(); } }
...
(function () {
  function safeInit() {
    try {
      // ensure tabs link works (if HTML changed)
