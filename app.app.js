...
/** "1,84" -> 0.0184 (fracción mensual). Solo coma permitida. */
...
class Finanzas {
  ...
  ensureMonth(key){
    ...
    if(this.data[prev]){
      ...
      // reasignar ids y ajustar fechas; resetear paid a false
      Object.values(copy).forEach(arr=>{
        if(Array.isArray(arr)){
          arr.forEach(it=>{
            it.id=this.uid();
            it.fecha=`${key}-01`;
            if(typeof it.paid!=="undefined") it.paid=false;
          });
        }
      });
      this.data[key]=copy;
  ...
  cuota(M,i,n,avalPct=0,ivaAvalPct=0){
    ...
    let base;
    if(!i) base=M/n;
    else { const f=Math.pow(1+i,n); base=(M*i*f)/(f-1); }
  ...
  showModal(title, innerHtml, onSubmit){
    ...
    if(modal) modal.classList.remove("hidden"); if(modal) modal.style.display="flex"; if(modal) modal.setAttribute("aria-hidden","false");

    const cancelEl=document.getElementById("cancelModal");
    if(cancelEl) cancelEl.onclick=()=>this.closeModal();
